...
RSpec.describe Plans do
  ...
  describe "#basic_price" do
    ...
    context "2社の場合" do
      it "2社分のデータを返すこと" do
        plans = Plans.new([tokyo_gas, tepco])
  ...
  describe "#energy_price" do
    context "東京ガスの場合" do
      ...
      it "正しい電力量料金を返すこと(1ヶ月の使用量が141 ~ 350kWh)" do
        plans = Plans.new([tokyo_gas])
        expect(plans.energy_price(141)[0].values[0]).to eq (141 * 23.88)
        expect(plans.energy_price(350)[0].values[0]).to eq (350 * 23.88)
      end

      it "正しい電力量料金を返すこと(1ヶ月の使用量が351kWh以上)" do
        plans = Plans.new([tokyo_gas])
        expect(plans.energy_price(351)[0].values[0]).to eq (351 * 26.41)
      end
    end
    ...
    context "東京電力エナジーパートナーの場合" do
      ...
      it "正しい電力量料金を返すこと(1ヶ月の使用量が121 ~ 300kWh)" do
        plans = Plans.new([tepco])
        expect(plans.energy_price(121)[0].values[0]).to eq (121 * 26.48)
        expect(plans.energy_price(300)[0].values[0]).to eq (300 * 26.48)
      end

      it "正しい電力量料金を返すこと(1ヶ月の使用量が301kWh以上)" do
        plans = Plans.new([tepco])
        expect(plans.energy_price(301)[0].values[0]).to eq (301 * 30.57)
      end
    end
