...
describe Simulator do
  ...
  describe 'Loope おうちプラン' do
    it "calculates charge of Looop's homePlan" do
      ...
      expect(simulator.looop_homePlan).to eq expected_charge.floor
    end
  end
  ...
  describe '#ep_planB' do
    it " 東京電力従量電灯B 10~60A, 第二段階を超えない使用量" do
      ...
      expect(simulator.ep_planB).to eq expected_charge.floor
    end
  ...
  describe '#tg_zuttomoDenki1' do
    it "東京ガスずっとも電気１ 30~60A, 第二段階を超えない使用量" do
      ...
      expect(simulator.tg_zuttomoDenki1).to eq expected_charge.floor
    end
    ...
    context('引数に誤りがない場合') do 
      ...
      context("どのプランの規定にも変更がない場合") do
        it "puts an array of hashes summaring each plan's data" do
          simulator = Simulator.new(random_amp, random_usage)
          output = [
            {provider_name: "東京電力エナジーパートナー", plan_name: "従量電灯B", price: "#{simulator.ep_planB}"},
            {provider_name: "Looop電気", plan_name: "おうちプラン", price: "#{simulator.looop_homePlan}"},
            {provider_name: "東京ガス", plan_name: "ずっとも電気１", price: "#{simulator.tg_zuttomoDenki1}"}
          ]
