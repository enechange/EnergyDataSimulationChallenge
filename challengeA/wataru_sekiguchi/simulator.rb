class Simulator
  ...
  def set_info
    ...
    @info = [{ name: '東京電力エナジーパートナー', plan: '従量電灯B', base: tepco, rate: { 120 => 19.88, 300 => 26.48, 9**9 => 30.57 } },
  ...
  def simulate
    ...
    puts '**************************************************************'
    @current_bills <= @info.first[:total_price] ? no_change : change
    suggestion
  end
  ...
  def total_price(plan, prev_key = 0)
  ...
  def base_price(plan, prev_key = 0)
    plan[:base].each do |key, value|
      return @consumption.zero? ? (value * 0.5).floor : value if @amp > prev_key && @amp <= key
  ...
  def self.valid_value(comment, flag = nil)
    (1..2).each do |_n|
      $stdout.puts comment
      input = $stdin.gets.chomp
      return input.to_i if !(input =~ /^[0-9]+$/).nil? && flag.nil?
      return input.to_i if [10, 15, 20, 30, 40, 50, 60].include?(input.to_i)
    end
